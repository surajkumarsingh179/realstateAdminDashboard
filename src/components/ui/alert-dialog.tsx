...
>(({ className, ...props }, ref) => (
  <Dialog.Portal>
    ...
    <Dialog.Content
      ref={ref}
      className={cn(
        "fixed left-[50%] top-[50%] z-50 grid w-full max-w-md translate-x-[-50%] translate-y-[-50%] gap-4 bg-background border p-6 shadow-xl rounded-xl duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=open]:slide-in-from-left-1/2",
        className
      )}
      {...props}
    />
  ...
  ({ className, onClick, ...props }, ref) => {
    return (
      <Dialog.Close asChild>
        <button ref={ref} className={cn(buttonVariants({ variant: "destructive" }), className)} onClick={onClick} {...props} />
      </Dialog.Close>
    );
  }
);
